...
file_prefix = f"{coin_type}_{year}0{month}"
...
for date in sorted(daily_stats.keys()):
    with open("spammer.txt", 'a', encoding='utf-8-sig') as txtfile:
        txtfile.write(f"\n📅 {date}\n")
        txtfile.write(f"總作者數：{daily_stats[date]['authors']} 位\n")
        txtfile.write(f"總貼文數：{daily_stats[date]['tweets']} 篇\n")
    ...
    with open("spammer.txt", 'a', encoding='utf-8-sig') as txtfile:
        if fast_authors:
            txtfile.write("⚡ 發文太快的作者（1小時內6篇以上）：\n")
            # for user, count in sorted(qualified_authors_per_day[date], key=lambda x: -x[1]):
            #     txtfile.write(f"    {user}: {count} 篇\n")
            txtfile.write(f"🔢 這些快速作者合計貼文數：{fast_authors_total_posts} 篇\n")
        else:
            txtfile.write("⚠ 無符合『1小時內至少6篇且不只1篇』的作者\n")
    ...
    with open("spammer.txt", 'a', encoding='utf-8-sig') as txtfile:
        txtfile.write(f"📊 當天發文數最多的前 {rank_limit} 名：\n")
        # for i, (user, count) in enumerate(top_n_authors, start=1):
            # txtfile.write(f"    {i}. {user}: {count} 篇\n")
        txtfile.write(f"🔢 這些前{rank_limit}名合計貼文數：{top_n_total_posts} 篇\n")
    ...
    with open("spammer.txt", 'a', encoding='utf-8-sig') as txtfile:
        txtfile.write(f"📈 發文太快的作者比例：{fast_authors_count} / {total_authors} = {ratio_fast_authors:.2%}\n")
        txtfile.write(f"📈 發文太快作者的貼文比例：{fast_authors_total_posts} / {total_tweets} = {ratio_fast_posts:.2%}\n")
        txtfile.write(f"📈 當天前{rank_limit}名作者的貼文比例：{top_n_total_posts} / {total_tweets} = {ratio_topn_posts:.2%}\n")
