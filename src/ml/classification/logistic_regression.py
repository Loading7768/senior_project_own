...
# --- 訓練用函式 ---
def train_function(X_train, X_test, y_train, y_test, scaler, features_name, selector, pipeline_path):
    if RUN_FIRST_CLASSIFIER:
        ...
        with open(txt_path, "w", encoding="utf-8") as f:
            ...
            for coin, daily in combined_daily.items():
                ...
                if records:
                    ...
                    npy_path = f"{OUTPUT_PATH}/{coin}_combined_daily_predictions.npy"
...
def load_and_preprocess():
    if RUN_FIRST_CLASSIFIER:
        ...
        y_train_categorized = categorize_array_multi(y_train_all, T1, T2, T3, T4)  # shape (N,5)
        y_test_categorized  = categorize_array_multi(y_test_all, T1, T2, T3, T4)   # shape (N,5)
        print("已成功分類別")

        # 統計每個類別數量
        print(f"大跌：-{T1 * 100}%以下, 跌：-{T1 * 100}% ~ -{T2 * 100}%, 持平：-{T2 * 100}% ~ {T3 * 100}%, 漲：{T3 * 100}% ~ {T4 * 100}%, 大漲：{T4 * 100}%以上")
...
def main():
    ...
    if RUN_FIRST_CLASSIFIER:
        ...
        if IS_TRAIN:
            # --- 訓練模型 --- 
            train_function(X_train, X_test, y_train, y_test, scaler, features_name, selector, pipeline_path)
